...
public class GoblinSpawnerInformation : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    [SerializeField]
    ...
    void Update()
    {
        ...
        {
            cost = costObj.cost;
            health = monster.GetComponent<MonsterController>().maxHealth * upgrades.goblinHealthUpgrade;
            speed = monster.GetComponent<MonsterController>().moveSpeed;
            value = monster.GetComponent<MonsterController>().scoreValue * upgrades.goblinRewardUpgrade;
            spawnRate = spawner.GetComponent<MonsterSpawning>().spawnTime / upgrades.goblinSpawnUpgrade;
            infoText.transform.parent.gameObject.SetActive(true);
            infoText.text = "Goblin Spawner \n \n" + "Cost: " + cost.ToString() + "\n" +
