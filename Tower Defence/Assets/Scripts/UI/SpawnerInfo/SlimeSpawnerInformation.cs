...
public class SlimeSpawnerInformation : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    [SerializeField]
    ...
    void Update()
    {
        ...
        if (currentHover == gameObject || currentHover == nameText || currentHover == costText || currentHover == Image || currentHover == backGround)
        {
            ...
            value = monster.GetComponent<MonsterController>().scoreValue * upgrades.slimeRewardUpgrade;
            spawnRate = spawner.GetComponent<MonsterSpawning>().spawnTime / upgrades.slimeSpawnUpgrade;
            infoText.transform.parent.gameObject.SetActive(true);
            infoText.text = "slime Spawner \n \n" + "Cost: " + cost.ToString() + "\n" +
