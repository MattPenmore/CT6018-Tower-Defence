...
public class HammerInformation : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    [SerializeField]
    ...
    void Update()
    {
        if (currentHover == gameObject || currentHover == nameText || currentHover == costText || currentHover == Image || currentHover == backGround)
        {
            ...
            speed = tower.GetComponent<HammerController>().hitSpeed / upgrades.hammerSpeedUpgrade;
            infoText.transform.parent.gameObject.SetActive(true);
            infoText.text = "Hammer \n \n" + "Cost: " + cost.ToString() + "\n" +
