...
public class MachineGunInformation : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    [SerializeField]
    ...
    void Update()
    {
        ...
        {
            cost = costObj.cost;
            damage = tower.GetComponent<MachineGunController>().damage * upgrades.machineGunDamageUpgrade;
            speed = tower.GetComponent<MachineGunController>().shootSpeed / upgrades.machineGunSpeedUpgrade;
            range = tower.GetComponent<MachineGunController>().Range + upgrades.machineGunRangeUpgrade;
            infoText.transform.parent.gameObject.SetActive(true);
            infoText.text = "Machine Gun \n \n" + "Cost: " + cost.ToString() + "\n" +
